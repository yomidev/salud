...
function dselect(el, option = {}) {
  ...
  function itemTags(options) {
    ...
    for (const option of options) {
      if (option.tagName === 'OPTGROUP') {
        ...
        items.push(`<button${hidden} class="dropdown-item${active} dropdown-items" data-dselect-value="${value}" type="button" onclick="dselectUpdate(this, '${classElement}', '${classToggler}')"${disabled}>${text}</button>`)
      }
    }
    items = items.join('')
    return items
  ...
  function createDom() {
    ...
    <div class="dropdown ${classElement} ${additionalClass}">
      ...
      <div class="dropdown-menu" style="background-color:#fff !important; color:#000 !important;>
        <div class="d-flex flex-column">
          ${searchInput}
          <div class="dselect-items" style="max-height:${maxHeight};overflow:auto;color:#000 !important;">
            ${itemTags(el.querySelectorAll('*'))}
          </div>
          <div class="${classNoResults} d-none" style="color:#000 !important;">No results found</div>
        </div>
      </div>
      ${clearBtn}
    </div>
